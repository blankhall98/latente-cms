...
def list_tenants(
    ...
    if q:
        term = f"%{q.strip()}%"
        stmt = stmt.where(Tenant.name.ilike(term))
...
def create_tenant(
    ...
    t = Tenant(name=payload.name, slug=slug)
    db.add(t)
    db.commit()
    db.refresh(t)
    return t
...
def update_tenant(
    ...
    if patch.slug is not None:
        new_slug = patch.slug.strip().lower()
        # validate uniqueness
        existing = db.scalar(select(Tenant).where(Tenant.slug == new_slug, Tenant.id != tenant_id))
        if existing:
